...
def main() -> int:
    ...
    ap.add_argument("--effort", default="low",
                    help="output_config.effort: low|medium|high|xhigh|max, or 'none' to omit it. "
                         "Haiku 4.5 REJECTS the parameter outright (400 'This model does not "
                         "support the effort parameter'), so it needs --effort none.")
    ...
    for o in outs:
        ...
        row = {
            ...
            "offline_reward": mine["reward"] if mine else None,
            "outcome": mine["outcome"] if mine else "NO_INFO",
            "well_formed": mine["well_formed"] if mine else 0.0,
            "false_claim": mine["false_claim"] if mine else 0.0,
            "in_box_kill": mine.get("in_box_kill") if mine else None,
        ...
        rows.append(row)
        if mine is not None and abs(theirs - mine["reward"]) > 1e-9:
    ...
    rec = {
        "what": "The blind-spot environment run THROUGH verifiers against a live model, and the "
        ...
        "unparseable": sum(1 for r in rows if r["well_formed"] == 0.0),
        "false_claims": sum(int(r["false_claim"]) for r in rows),
        "avg_metrics": metrics,
...
def _by_rung(rows):
    out = {}
    for r in rows:
        b = out.setdefault(r["rung"], {"n": 0, "reward": 0.0, "solved": 0, "wrong": 0, "unparseable": 0})
        b["n"] += 1
        b["reward"] += r["offline_reward"]
        b["solved"] += int(r["offline_reward"] >= 1)
        b["wrong"] += int(r["offline_reward"] < 0)
        b["unparseable"] += int(r["well_formed"] == 0.0)
    for b in out.values():
