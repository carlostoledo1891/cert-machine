#!/usr/bin/env node
/* build-terra-pdf.js — render paper/terra-peaks.md to paper/terra-peaks.pdf.

   The write-up is generated from certificates by tools/build-terra-writeup.js;
...
const OUT = path.join(ROOT, 'paper', 'terra-peaks.pdf');
const CHROME = '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';
const die = (m) => { console.error('TERRA PDF REFUSED: ' + m); process.exit(1); };
if (!fs.existsSync(MD)) die('paper/terra-peaks.md missing — run tools/build-terra-writeup.js first');
...
const html = '<!doctype html><html><meta charset="utf-8"><title>terra-peaks</title>'
...
const tmp = path.join(os.tmpdir(), 'terra-peaks-print-' + process.pid + '.html');
...
async function main() {
  const chrome = cp.spawn(CHROME, ['--headless=new', '--remote-debugging-port=9222', '--hide-scrollbars',
    '--user-data-dir=' + fs.mkdtempSync(path.join(os.tmpdir(), 'terra-pdf-'))], { stdio: 'ignore' });
  for (let t = 0; t < 60; t++) {
  ...
  chrome.kill();
  fs.unlinkSync(tmp);
  console.log('wrote paper/terra-peaks.pdf (' + (fs.statSync(OUT).size / 1024).toFixed(0) + ' KB)');
}
