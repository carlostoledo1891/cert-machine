...
const foot = '<p>Generated by tools/build-report-navierstokes.js @ git ' + git + '. Gates at this build: lean-repo.json recomputed from the pinned clone (' + R.commit.slice(0, 8) + '), build.json with a finished build and four kernel axiom lines, probes.json PASS, the manifest\'s digest of the paper matched, the solution-side definitions byte-identical to the challenge, every fidelity row same or equivalent, the energy-clause finding present in the record. A differing definition, a non-standard axiom, a failing probe or a missing record refuses this page.</p>';
