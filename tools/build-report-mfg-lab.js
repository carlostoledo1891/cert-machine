...
/* ---- the reproduction, edge by edge --------------------------------------
   "Matches Table I within its rounding" is a single number standing in for
   fifteen comparisons. The comparisons are right there in the battery's own
   output, so draw them: the reader can see which edges land exactly and which
   differ, and by how much. */
{
  const gl = (re, what) => { const m = re.exec(wd); if (!m) die('battery output lost the ' + what + ' line'); return m[1]; };
  const ours = gl(/^\s*totals:\s*([\d.,\s]+)$/m, 'reproduced totals').split(',').map(Number);
  const paper = gl(/^\s*paper\s*:\s*([\d.,\s]+)$/m, 'Table I totals').split(',').map(Number);
  if (ours.length !== paper.length) die('the totals and Table I rows no longer line up');
  const diff = ours.map((v, i) => Math.abs(v - paper[i]));
  const exact = diff.filter(d => d === 0).length;
  const worst = Math.max.apply(null, diff);
  const rowsFig = ours.map((v, i) => ({ k: 'edge ' + (i + 1), a: paper[i], b: v,
    lab: diff[i] === 0 ? 'same value' : '\u0394 ' + diff[i].toFixed(0) }));
  const lo = Math.min.apply(null, ours.concat(paper)), hi = Math.max.apply(null, ours.concat(paper));
  const fig = CH.dumbbell({
    w: 900, rowH: 26, x0: 0, x1: hi * 1.06, padL: 96, padR: 92,
    xTicks: [0, 25, 50, 75, 100].filter(v => v <= hi * 1.06).map(v => ({ v, t: String(v) })),
    rows: rowsFig,
    aName: 'Table I, as printed', bName: 'reproduced here, then certified (rounded the same way)',
    vOf: v => 'total flow ' + v,
    alt: 'Fifteen edges of the published Wardrop network. For each, the flow printed in Table I and the flow '
      + 'this build reproduced. ' + exact + ' of the ' + ours.length + ' land on the same value; the rest differ '
      + 'by at most ' + worst.toFixed(0) + ' units, inside the rounding of the printed table.'
  });
  B.push(C.section({
    lab: '§0 · the reproduction', title: 'The published table, edge by edge',
    wide: true,
    bodyRaw: '<div class="col">'
      + C.pRaw('Each row is one edge of the paper\'s network: the flow as Table I prints it, and the flow this '
        + 'build recomputed and then certified. Where the two dots sit on top of each other the reproduction is '
        + 'exact; where they separate, the gap is the paper\'s own rounding, and it is drawn to scale.')
      + '</div>'
      + C.figure({ svgRaw: fig, caption: exact + ' of ' + ours.length + ' edges land on the same printed value. '
        + 'Both series here are the battery\'s own ROUNDED totals, so the largest gap on the chart is '
        + worst.toFixed(0) + ' whole units; measured before rounding, the worst deviation across all '
        + ours.length + ' edges is ' + tblDev[1] + ', which is the number the gate actually enforces against the '
        + 'paper\'s stated tolerance of 2. Either way every difference is inside what the paper itself claims. '
        + 'This is what a reproduction should look like '
        + 'like when it is stated honestly: not "matches", but fifteen comparisons, each visible, with the '
        + 'largest named. The totals are unique across reseeds; the SPLIT behind them is not, which is the '
        + 'finding the rest of this page is about.' })
  }));
}
...
const foot = '<p>Generated by tools/build-report-mfg-lab.js @ git ' + git + '. Four of the '
  ...
  + 'legacy/research/mfg-lab/ in this repository; original artifact: research/mfg-lab in the public mfg-lab tree.</p>';

fs.writeFileSync(path.join(ROOT, 'reports', 'mfg-lab.html'),
  TPL.render({ title: 'The MFG laboratory, certified', bodyRaw: B.join('\n\n') + CH.script(), footRaw: foot, path: '/reports/mfg-lab.html',
    desc: 'A single-file mean-field-games laboratory, certified: a published Wardrop table reproduced and proved by Krawczyk boxes and exact rational solves at every build.' }));
