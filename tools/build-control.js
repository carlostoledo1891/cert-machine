#!/usr/bin/env node
/* build-control.js — generate control.html from what the machine produced.
...
B.push(C.stats([
  ...
  { k: 'batteries green', v: green + ' / ' + ran, role: 'held', n: 'Executed during this build. 19 red controls must fire.' }
]));
...
if (ledger.families.length) {
  ...
  B.push(C.section({
    lab: '§1 · the families', title: 'What the engine is enumerating', wide: true,
...
if (ledger.conjectures.length) {
  ...
  B.push(C.section({
    lab: '§2 · certified conjectures', title: 'The objects that survived', wide: true,
...

{
  ...
  B.push(C.section({
    lab: '§3 · closed forms', title: 'What survived the enclosure test', wide: true,
    bodyRaw: (rows.length
...

{
  ...
  B.push(C.section({
    lab: '§4 · the envelope', title: 'What a Newman hit has to beat', wide: true,
...

{
  ...
  B.push(C.section({
    lab: '§5 · the instruments', title: 'What certifies, and whether it runs', wide: true,
...
fs.writeFileSync(path.join(ROOT, 'control.html'),
  TPL.render({ title: 'cert-machine · the conjecture engine', bodyRaw: B.join('\n\n'), footRaw: foot }));

console.log('control.html written');
