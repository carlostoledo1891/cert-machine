...
const BATTERIES = [
  ...
  ['easota (the SOTA table)', ['instruments/easota/battery.js'], 'The EinsteinArena / Together AI "new SOTA" table re-decided in exact rationals: the reader, every decider on a closed-form calibration, and reds that must fire — an overlapping pair, a box over the perimeter, a collinear triple, a repeated point, a value above 1, a zero row, a negative value; then the shipped ledger walked (20 rows, 16 WITNESSED, 4 REPAIRED, eight improvements decided real, two suprema certified, two hexagon packings certified in interval arithmetic) and one row re-decided live from the pinned bytes.'],
  ['ecbench (contour benchmark)', ['instruments/ecbench/battery.js'], 'The environmental-contour benchmark (Haselsteiner et al. 2021) re-decided in exact plane geometry: the literal reader, the float-filtered orientation predicate with its proved bound and a planted case where float64 gives the wrong sign, a square\'s edges and corners ON, a concave shape, and reds that must fire — a bowtie, a square traced twice, a fold, a NaN, a column swap, a point 1e-20 off an edge; Table 1\'s erfc enclosure against reference values; then the shipped ledger walked (167 pins re-hashed, 150 contours, 176 printed numbers: 173 exact, one within the two boundary points, two not the file\'s) and four contours re-decided live against the pinned hourly data.'],
  ['stereo (rig error budget)', ['instruments/stereo/battery.js'], 'The error budget of a stereo-video wave rig as enclosures (instruments/stereo/budget.js over the interval library): a 3-4-5 geometry whose cell closes on the true point with the analytic width, the exact cell never narrower than the textbook linearisation, the bound growing with range, the reach a prefix of its grid, and reds that must fire — a zero baseline, a negative lag, a zero range, a disparity box reaching zero (refused, not clamped), an impossible tolerance; then the Leme 2020 rig\'s facts as the instrument page states them: every observed RMSE of its Table 1 between the best and worst corners of the budget, and the quoted quantization below the best-case cell.'],
  ['breaking (Black Sea waves)', ['instruments/breaking/battery.js'], 'The Black Sea breaking-wave table (16,369 events; Zenodo 2026) decided in exact rationals — the literal reader, order statistics as the k-th smallest value, average ranks with ties, Pearson and Spearman as exact r² with r enclosed by integer square roots — and, since 2026-09-17, the paper beside it (Guimarães et al., GRL 10.1029/2026GL122293) read number by number: rounding verdicts, fits through the origin, enclosed moments, the Freedman–Diaconis mode, the 4 % tail; reds that must fire — a NaN, a comma decimal, a reversed line\'s sign, one literal moved by 1e-17, a printed 3 against 1.376, a Table 1 count moved by one, a bin count that straddles an integer; then the shipped ledger walked and the whole table and paper re-decided live.'],
  ['hseva (certified return levels)', ['instruments/hseva/battery.js'], 'Five wave-height families (exponential, normal, lognormal, Weibull, exponentiated Weibull) written once over an abstract arithmetic: Φ and Φ⁻¹ enclosed (Taylor with a proved tail, Laplace\'s continued fraction), every score checked against finite differences of its likelihood and every Hessian against its score, the exponentiated Weibull collapsing to the Weibull at α = 1, quantile ∘ cdf the identity; Krawczyk certificates on samples with known answers; reds that must fire — Φ(1) is not 0.85, overlapping A² enclosures refuse a ranking, a constant sample has no Weibull fit; then the shipped ledger walked and its daily, monthly and annual blocks re-certified live on the three buoys.'],
  ...
  ['monoflow (monotone, not a gradient)', ['instruments/monoflow/battery.js'], 'the Lasry–Lions monotone flow of the quadratic MFG at seven certified equilibria: the linearised form returns c/2 from the assembled interval Jacobian on every instance, and at five of them an eigenvalue of the flow Jacobian is enclosed with Im μ away from 0 — not a gradient flow under any metric; the closed-form mode at A = 0 is the control · 5 red controls'],
  ['aag (the empty region, painted)', ['instruments/aag/battery.js'], 'Alharbi–Ashrafyan–Gomes §3.2 re-decided live: the vanishing set cut into cells that are OCCUPIED, EMPTY or REFUSED (each refused cell contains one of the three exact roots, refused length 4/K at every budget), the two value functions enclosed and distinct, the contact set with and without exit evaluated as intervals, case 2 positive over a γ box · 5 red controls'],
  ['maxval (the maximal value function)', ['instruments/maxval/battery.js'], 'Gomes–Üçer Theorem 1.8 drawn on an explicit first-order game with a vacuum, built here: the parabolic-bump pair re-verified cell by cell (HJ and transport residuals enclose 0 on the support, strict subsolution off it), the maximal value function enclosed on every vacuum cell between the free Hopf–Lax value and the cheapest path proved clear of the support, u* ≥ u wherever both are decided · 5 red controls'],
  ['frontier (the concentration frontier, measured)', ['instruments/frontier/battery.js'], 'the congestion-MFG certifier\'s refusal frontier re-derived from three sha-pinned data files lifted from the source lab: six monotone σ ladders at N = 14 with bisected A⋆ brackets and named refusals, A⋆ rising 5–10 % from N = 14 to 40, the N-free ceiling A_rec bit-identical across N, ratios below 1; a pin that moves refuses · 5 red controls'],
  ['price (the clearing band)', ['instruments/price/battery.js'], 'Gomes–Saúde price formation (arXiv:1807.07088), linear-quadratic: the clearing price closed in exact rationals on the source lab\'s supply scenario — Π_n = −Θ and Ξ(T) = x̄₀ + ∫Q as rational identities, a ±15 % forecast box turned into a price band attained at its corners with twenty interior paths inside it, the closed form against an independent Riccati route to 1e−14 — and the lab\'s finite-difference kernel ported bit for bit and measured: it clears in the controls while 12 % of the day\'s energy vanishes at its wall; flux clearing conserves and converges at first order · 5 red controls'],
  ['agtable (their tables, re-decided)', ['instruments/agtable/battery.js'], 'Ashrafyan–Gomes Tables 1 and 2 (arXiv:2403.02785, the semi-Lagrangian price-formation scheme): the two analytic solutions the paper tests against enclosed in interval arithmetic (a new second-order interval jet, instruments/interval/taylor2.js, bounds every integral\'s remainder), the clearing identity ϖ + a₁ + 2a₂K = −Q decided at 21 times, the scheme written from the paper and run on the paper\'s meshes: the price column of test 1 reproduced to its two digits at every mesh, test 2 reproduced except where the port does better, the tolerance found to be 39 % of the finest printed number, and the printed initial density shown not to be a density as printed · 5 red controls'],
  ['regatlas (the regularization atlas)', ['instruments/regatlas/battery.js'], 'Ferreira–Gomes–Üçer\'s p-Laplacian regularization (arXiv:2506.21212, operator (3.1)) on the one-dimensional discounted first-order MFG with H = |p|²/2 − m: a 10 × 5 atlas in (A, ε), every cell a unique classical even solution with positive density enclosed by a radii polynomial in a derivative-weighted ℓ¹ space (a NEW kernel: Sturm–Liouville linearisation, explicit columns to 6N, analytic tail) or refused by name; ε = 0 certifies as far as any ε > 0 — the game is elliptic after eliminating m — and the distance ‖u_ε − u_0‖ is decided as a bracket, ≈ ε at small ε · 5 red controls'],
  ['hbar (the effective Hamiltonian band)', ['instruments/hbar/battery.js'], 'the one-dimensional effective Hamiltonian H̄(P) of Gomes–Yang\'s test case (arXiv:1810.03483 §6) enclosed as a band: P₀ = ∫√(2(1 − V)) enclosed around 4/π, the flat part decided exactly, the rotating part bracketed by bisection on a midpoint rule whose remainder a second-order interval jet bounds, the degeneracy at P₀ met as a widening band and one undecided point, the Mather density enclosed where it rotates; Table 1\'s separable value enclosed below 1e−6 with the quoted Gomes–Oberman 4.4099660 found ABOVE it, Table 2\'s exact value decided as 1 · 5 red controls'],
...
const PY = [
  ...
  ['blind-spot (the chip environment)', ['environments/blind_spot/battery.py'], 'the chip-task environment ported from frontier-apps 2026-09-09 and re-derived on every build: the 27 lifted files re-hashed against their pins, MCY\'s 400 mutations of a Euclidean-norm comparator checked against the SAT record (348 killable, 51 proved equivalent, the identity found by -mode none \u2014 MCY numbers it 1, not 0), EVERY ONE of the 348 recorded witnesses re-run through the actual netlist and required to flip a pin, eight proved-equivalent mutants attacked with all four testbench families (16,000 pairs each) and required to survive, and the 11 planted controls of which THREE MUST SCORE \u2014 a suite that fails everything reports perfect coverage, which is the mistake the environment is named for. The SAT labelling itself runs once, on the port, and is a pinned record; the simulation is re-derived \u00b7 4 red controls, one of them the vacuous identity query that went quiet in the source lab for a whole session'],
  ['blind-spot inspect (one scorer, two frameworks)', ['environments/blind_spot/inspect/battery.py'], 'blind-spot as an inspect_ai Task (2026-09-21, the METR plan\'s first deliverable): the scorer is blind_spot.adapters_v0._decide, the function the verifiers rubric calls, applied to Inspect\'s own messages — nothing re-implemented. Every build: the three rung variants and the mixed task built and their ids checked against the taskset; the Inspect scorer, api.score and taskset.grade scored on EVERY one of the 400 pooled mutants (truthful, UNDECIDED, and the false claim on all 52 proved-equivalent plus 40 killable) and required to agree field for field; every recorded Inspect log re-scored offline with 0 disagreements; the ledger\'s pins (task.py, baselines.json, every log) re-hashed and its counts re-derived; the README\'s quoted numbers found in the ledger; the human-baseline attempts re-graded · 4 red controls: a witness with one coordinate changed stops killing under BOTH scorers wherever it stops (MISSED, never SOLVED; on the mutants indifferent to that coordinate it still kills and both say so — the two never split; 25 tried), a killing verdict buried in a reasoning block grades UNDECIDED both ways round (the text-only filter is load-bearing), a coordinate no 3-bit pin can carry is refused rather than masked'],
  ['gsm8k (the answer key)', ['instruments/gsm8k/battery.py'], 'GSM8K\'s answer key re-decided in exact rational arithmetic (2026-09-21, the METR plan\'s third deliverable): the exact reader calibrated on cases with known answers (the calculator grammar, leading-dot decimals, thousands separators, a percent as a hundredth, mixed numbers, a spaceless fraction as one literal, running chains, unit labels), every one of the 4,282 test and 23,716 train calculator annotations and every readable prose equation evaluated from the expression it prints, the three pinned corpus files re-hashed, the ledger re-derived unchanged, the GSM8K-Platinum join re-counted against its printed 110 / 99 / 10 · 12 red controls: a planted wrong annotation, a value off by more than half a unit in its last place, division by zero, the // operator, a Python name and a power (nothing is ever eval-ed), an annotation without one =, the two test-set witnesses, a wrong step whose answer rests on it, a refused item, a trailing period'],
  ['horizon (certified fits)', ['instruments/horizon/battery.py'], 'METR\'s time horizon as a certificate (2026-09-22, the METR plan\'s deliverable 2 instrument, calibrated on METR\'s own Time Horizon 1.1 evidence): a Python interval library proved to contain (exp, log, 2^x against 20-digit expansions and 600 random points; a series with its tail bound dropped as a red), the penalised logistic fit certified by the Krawczyk operator on synthetic data with a known law (slope, intercept and the 32-minute horizon recovered), per-run and per-task fits shown to certify the same horizon, METR\'s weighting rule re-derived, the pinned evidence re-hashed, one agent re-certified live from the pinned per-task file and required to give the ledger\'s box, the post-2023 doubling time re-read as an enclosure within 5 % of the site\'s printed value · 6 red controls: division by an interval through zero, a log touching zero, a Taylor series without its remainder, Krawczyk from a far candidate, a slope box through zero asked for a horizon, a printed horizon moved by 1 %'],
  ['erdos1 (the explicit sets)', ['instruments/erdos1/battery.py'], 'Erdős #1 made effective, ported from frontier-apps 2026-09-15 and re-derived at every build: the 66 lifted files re-hashed against their pins; Δ_s = |det B_s| exactly on small lattices and for three tilts; the cube property of Λ_1 by exhaustive search for d ≤ 9; the structured strip decision (Lemma 1″) against a brute-force decision of both gadget properties for b ≤ 7 and four tilts, and every (α, b) a certificate uses; verify.py (V0–V7, FLINT) on the two smallest certificates; the ledger’s verified set re-read from the logs shared-nothing, N/2^n and the Siegel bound re-derived from the base weights, siegel.json against the ledger; the detached verifier byte-identical to its generator · 6 red controls: a forged pin, a base weight off by one (V4b), a halved buffer (V5), a Hermite entry off by one (V2), a gadget that violates the cube condition, a verifier log that did not end green'],
