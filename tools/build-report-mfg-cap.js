...
/* ---- disjointness, to scale ----------------------------------------------
   "Two disjoint balls" is a claim about three numbers in ONE unit — the
   nu-weighted distance — so they belong on one axis. Drawn logarithmically the
   claim stops being a sentence: the gap between the two solutions is thirteen
   decades wider than the balls that enclose them. */
{
  const sep = separation, rs = Number(rsum), ctrl = Number(p1[1]);
  const lo = Math.min(rs, ctrl) / 8, hi = sep * 8;
  const fig = CH.bars({
    w: 900, rowH: 34, logX: true, min: lo, max: hi, padL: 300, padR: 128,
    rows: [
      { k: 'distance between the two solutions', v: sep, lab: sep.toFixed(3),
        hover: 'nu-weighted distance between the two certified centres' },
      { k: 'combined radius of the two balls', v: rs, lab: rs.toExponential(2),
        token: 'var(--c-2)', hover: 'r1 + r2 — the two enclosures together' },
      { k: 'worst enclosure in the monotone control', v: ctrl, lab: ctrl.toExponential(2),
        token: 'var(--c-2)', hover: 'the loosest radius across the five Lasry-Lions parameter sets' }
    ],
    xTicks: CH.decades(lo, hi, 7),
    xLabel: 'nu-weighted distance (log scale) — one unit, three quantities',
    alt: 'Three quantities in the same weighted norm on a logarithmic axis. The distance between the two '
      + 'certified solutions is ' + sep.toFixed(3) + '; the two enclosing balls have a combined radius of '
      + rs + ', about thirteen orders of magnitude smaller.'
  });
  B.push(C.section({
    lab: '§0 · to scale', title: 'How far from touching the two enclosures are',
    wide: true,
    bodyRaw: '<div class="col">'
      + C.pRaw('Disjointness is what makes this a multiplicity theorem rather than an observation, so it is '
        + 'worth seeing how much room there is. All three quantities below are lengths in the same weighted '
        + 'norm, so they share one axis; it is logarithmic because otherwise two of the three bars would be '
        + 'invisible.')
      + '</div>'
      + C.figure({ svgRaw: fig, caption: 'The two certified solutions sit ' + sep.toFixed(3) + ' apart while '
        + 'their enclosures together span ' + rs + ' — a factor of about '
        + '10' + String(Math.round(Math.log10(sep / rs))).replace(/\d/g, d => '\u2070\u00b9\u00b2\u00b3\u2074\u2075\u2076\u2077\u2078\u2079'[+d])
        + '. Nothing about this proof is marginal: '
        + 'the balls would have to grow by thirteen decades before they touched. The third bar is the loosest '
        + 'enclosure the monotone control produced across five Lasry-Lions parameter sets, for scale — the '
        + 'classical regime certifies just as tightly, which is why the interesting question was never '
        + 'precision but whether TWO solutions exist at all.' })
  }));
}
...
const foot = '<p>Generated by tools/build-report-mfg-cap.js @ git ' + git + '. The unit\'s '
  ...
  + 'legacy/research/mfg-cap/ in this repository; original artifact: research/mfg-cap in the public mfg-lab tree.</p>';

fs.writeFileSync(path.join(ROOT, 'reports', 'mfg-cap.html'),
  TPL.render({ title: 'Certified multiplicity for a non-monotone MFG', bodyRaw: B.join('\n\n') + CH.script(), footRaw: foot, path: '/reports/mfg-cap.html',
    desc: 'Certified multiplicity for a non-monotone mean-field game: two equilibria enclosed in disjoint interval-arithmetic balls where uniqueness theory is silent.' }));
