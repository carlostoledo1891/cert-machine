...
const foot = '<p>' + C.esc('Generated by tools/build-report-mfg-congest.js @ git ' + gitrev
...
fs.writeFileSync(path.join(ROOT, 'reports', 'mfg-congest.html'),
  TPL.render({ title: 'A congestion mean-field game, enclosed · cert-machine', bodyRaw: O.join('\n\n') + CH.script(), footRaw: foot, path: '/reports/mfg-congest.html',
    desc: 'A congestion mean-field game decided by validated numerics: the equilibrium enclosed in an interval ball, and the one inequality the proof rests on, explicit.' }));
