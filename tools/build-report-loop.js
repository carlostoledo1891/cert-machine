...
/* ---- every trajectory, round by round -------------------------------------
   The loop's honest finding is negative and easy to bury in a table: the
   certified trajectories certified on their FIRST round, so the feedback
   never converted anything. Drawn round by round it cannot be buried. */
{
  const TOK = { certified: 'var(--c-2)', refuted: 'var(--c-1)', rejected: 'var(--c-3)',
                malformed: 'var(--c-ctx)', undecided: 'var(--c-ctx)', declined: 'var(--c-ctx)' };
  const HATCH = { undecided: true, declined: true };
  const maxR = Math.max.apply(null, trajs.map(t => t.rounds.length));
  const short = (m) => m.replace(/^claude-/, '').replace(/-\d{8}$/, '');
  const rowsFig = trajs.map(t => ({
    /* every trajectory here shares one target, so the target alone is not a
       label — the trajectory id is what tells two rows apart */
    k: short(t.model) + ' · ' + t.target + ' #' + t.trajectory,
    segs: t.rounds.map((r, i) => ({ x0: i, x1: i + 1, token: TOK[r.outcome] || 'var(--c-ctx)',
      hatch: !!HATCH[r.outcome], k: short(t.model) + ' · round ' + (i + 1), v: r.outcome }))
  }));
  const used = [...new Set(trajs.flatMap(t => t.rounds.map(r => r.outcome)))];
  const fig = CH.segments({
    w: 900, rowH: 32, x0: 0, x1: maxR, padL: 250,
    xTicks: Array.from({ length: maxR }, (_, i) => ({ v: i + 0.5, t: 'round ' + (i + 1) })),
    xLabel: 'rounds of the conversation, in order — one cell per graded proposal',
    rows: rowsFig,
    keys: used.map(o => ({ token: TOK[o] || 'var(--c-ctx)', t: o, kind: HATCH[o] ? 'hatch' : undefined })),
    alt: trajs.length + ' trajectories, one row each, one cell per round coloured by what the grader returned. '
      + 'The ' + nCert + ' that end certified do so in their first cell; no row turns green after a red one.'
  });
  B.push(C.section({
    lab: '§0 · the loop, drawn', title: 'Where the feedback did and did not convert',
    wide: true,
    bodyRaw: '<div class="col">'
      + C.pRaw('Each row is one conversation with the grader; each cell is one graded proposal. A row that '
        + 'turns green after a red cell is a conversion — the verifier\'s own mechanism, fed back, moving a '
        + 'model from wrong to proved. Look for one.')
      + '</div>'
      + C.figure({ svgRaw: fig, caption: trajs.length + ' trajectories, ' + totalRounds + ' graded rounds. '
        + nCert + ' end certified and ' + nConv + ' of those were conversions — every certified row is green in '
        + 'its first cell. That is the honest state of this loop and it is recorded exactly as a success would '
        + 'be: the feedback is template-locked to the grader\'s mechanism, so there is no coaching to credit, '
        + 'and so far there is no conversion to credit either.' })
  }));
}
...
const foot = '<p>Generated by tools/build-report-loop.js @ git ' + git + '. The grader\'s red '
  + 'and green controls re-ran during this build; every trajectory row was read off the append-only ledger; and '
  + 'every feedback string was checked against the fixed mechanism templates — a deviation refuses the page.</p>';

fs.writeFileSync(path.join(ROOT, 'reports', 'verifier-loop.html'),
  TPL.render({ title: 'The verifier in the loop', bodyRaw: B.join('\n\n') + CH.script(), footRaw: foot, path: '/reports/verifier-loop.html',
