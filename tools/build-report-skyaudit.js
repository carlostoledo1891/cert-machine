...
/* ---- the reserve ladder --------------------------------------------------
   The one chart this day earns. Four aircraft, one axis, and the shape IS the
   finding: three of them are at zero before the rule's own reserve begins, and
   the fourth walks down to zero at 30 minutes. Emphasis, not four hues — the
   story is one line and three that never leave the floor. */
{
  const MINS = [5, 10, 15, 20, 25, 30];
  const RL = opt.reserve_price.by_reserve_minutes;
  const NAME = { 'beta-alia': 'Beta ALIA-250', 'joby-s4': 'Joby S4',
                 'archer-midnight': 'Archer Midnight', 'eve-100': 'Eve 100' };
  const ser = [];
  for (const k of ['joby-s4', 'archer-midnight', 'eve-100']) {
    ser.push({ name: NAME[k], token: 'var(--c-ctx)', pts: MINS.map(m => [m, RL[k][String(m)]]) });
  }
  ser.push({ name: NAME['beta-alia'], pts: MINS.map(m => [m, RL['beta-alia'][String(m)]]),
             area: true, endLabel: String(RL['beta-alia']['30']) });
  const rule = S.flights;
  const fig = CH.lines({
    w: 900, h: 320, x0: 5, x1: 30, y0: 0, y1: Math.max(rule, RL['beta-alia']['5']) * 1.06,
    xTicks: MINS.map(m => ({ v: m, t: m + ' min' })),
    yTicks: [0, 100, 200, 300, rule].map(v => ({ v, t: fmt(v) })),
    xLabel: 'energy reserve held back at cruise power  (the FAA VFR rule sits at 20 minutes)',
    yLabel: 'flights PROVABLY flyable',
    series: ser,
    keys: [{ token: 'var(--c-1)', t: NAME['beta-alia'] + ' — the only aircraft provable past 10 minutes', kind: 'line' },
           { token: 'var(--c-ctx)', t: 'Joby S4 · Archer Midnight · Eve 100 — zero from 15 minutes on', kind: 'line' }],
    rules: [{ v: rule, t: 'all ' + fmt(rule) + ' flights in the day', token: 'var(--c-grid)' }],
    bands: [{ x0: 19.4, x1: 20.6, token: 'var(--c-grid)', t: 'the rule' }],
    xOf: m => 'reserve ' + m + ' min',
    vOf: v => v + ' flights certified',
    alt: 'Certified flights against the reserve held back, from 5 to 30 minutes. Beta ALIA-250 falls from '
      + RL['beta-alia']['5'] + ' at 5 minutes through ' + RL['beta-alia']['20'] + ' at the FAA\'s 20 minutes to '
      + RL['beta-alia']['30'] + ' at 30. Joby, Archer and Eve are already at or near zero by 10 minutes and flat '
      + 'on zero after that.'
  });
  B.push(C.section({
    lab: '§0 · the price of the rule', title: 'What a reserve minute actually costs',
    wide: true,
    bodyRaw: '<div class="col">'
      + C.pRaw('The reserve is the regulator\'s dial, so the honest way to show an energy audit is to sweep it. '
        + 'Every point is a re-decision of the whole day at that reserve — exact-corner proofs over each maker\'s '
        + 'own published boxes, not one model re-scaled.')
      + '</div>'
      + C.figure({ svgRaw: fig, caption: 'Beta ALIA-250 covers ' + RL['beta-alia']['5'] + ' of the day\'s '
        + fmt(S.flights) + ' flights with a 5-minute reserve, ' + RL['beta-alia']['20'] + ' at the FAA\'s 20 '
        + 'minutes, ' + RL['beta-alia']['25'] + ' at 25, and ' + RL['beta-alia']['30'] + ' at 30 — the rule as '
        + 'written is close to the edge of what today\'s published specs can prove. The other three publish too '
        + 'little to certify a single flight past 10 minutes, which is a statement about DISCLOSURE, not about '
        + 'their aircraft: the verdict is what their own numbers can be made to prove.' })
  }));
}
...
const foot = '<p>Generated by tools/build-report-skyaudit.js @ git ' + git + '. Gates at this '
  ...
  + 'deviation. App: apps/skyaudit · live at /apps/skyaudit/.</p>';

fs.writeFileSync(path.join(ROOT, 'reports', 'skyaudit.html'),
  TPL.render({ title: 'SkyAudit: the helicopter day, decided', bodyRaw: B.join('\n\n') + CH.script(), footRaw: foot, path: '/reports/skyaudit.html',
    desc: 'SkyAudit\'s citable methodology: one pinned day of New York helicopter traffic re-flown on paper by four eVTOLs\' own published numbers under the FAA reserve rule — every flight decided or honestly refused.' }));
