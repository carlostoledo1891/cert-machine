...
## Status, priority, and reproducibility

Published from the repository (carlostoledo.co, with the archived release
DOI); not peer-reviewed; not independently rerun. Two priority claims,
each stated with its fence and open to refutation:

1. To our knowledge, these are the **first certified equilibria of a
   mean-field game whose exact peak count strictly exceeds the
   potential's well count**. The nearest published phenomena are
   spontaneous instability (arXiv:2605.20213) and peaks that mirror the
   potential (Cesaroni-Cirant, arXiv:1705.10741); neither carries
   validated-numerics certificates, and we found no validated-numerics
   equilibrium enclosures for mean-field games anywhere in the
   literature.
2. The base instance of this program — the σ = 1/2 single-harmonic
   enclosure of \`reports/mfg-congest.html\`, released together with this
   atlas — is, to our knowledge, the **first validated-numerics enclosure
   of a mean-field-game equilibrium at all**: an exact solution within an
   explicit radius, locally unique in the full sequence-space ball, with
   strictly positive density.

Every certificate re-runs from the repository
...
The companion page's embedded stdlib verifier serves as the calibration
gate of every certificate here; the two are one program, released
together, and the priority claims above are made by that joint release.
...
/* the priority gates (added 2026-09-02 after the release audit found the
   planned firsts had been dropped): both claims must be present, each with
   its qualifier and fences, and neither may inflate */
if (!/to our knowledge/i.test(doc)) die('the priority claims lost their qualifier');
if (!/first certified equilibria of a\s+mean-field game whose exact peak count/i.test(doc)) die('the peak-count first is missing');
if (!/first validated-numerics enclosure\s+of a mean-field-game equilibrium/i.test(doc)) die('the base-instance first is missing');
if (!/2605\.20213/.test(doc) || !/1705\.10741/.test(doc)) die('a priority claim appears without its fences');
