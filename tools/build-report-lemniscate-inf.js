...
B.push(C.header({
  ...
  deck: 'Erdős, Herzog and Piranian asked in 1958 how small the set where a monic polynomial stays below 1 can '
    + 'be. This page brackets the infimum in certified interval arithmetic, both ends unconditional, and answers a '
    + 'model problem from Tao’s notes for every ε. Three AI-assisted proofs of the exact value are currently '
    + 'claimed and none has been independently examined, so nothing here assumes any of them.'
}));
...
const foot = '<p>Generated by tools/build-report-lemniscate-inf.js @ git ' + git + '. '
  ...
  + 'and terra theorems. Published, not peer-reviewed, not independently rerun.</p>';
...
fs.writeFileSync(path.join(ROOT, 'reports', 'erdos1038-inf.html'),
  TPL.render({ title: 'Erdős #1038: the infimum, bracketed', bodyRaw: B.join('\n\n') + CH.script(), footRaw: foot,
    ...
    desc: 'A certified unconditional bracket ' + lo + ' <= inf <= ' + up.toFixed(7) + ' for the Erdős-Herzog-Piranian '
      + 'infimum (Erdős #1038), with Tao\'s model Problem 4.1 answered for every epsilon — assuming none of the three '
      + 'currently claimed proofs of the exact value.' }));
