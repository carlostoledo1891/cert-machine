...
fs.writeFileSync(path.join(ROOT, 'reports', 'verify_wardrop.py'), code);   /* the extracted verifier ships beside the page */
...
/* ---- one paper, three verdicts -------------------------------------------
   The title of this page is a claim about grammar: exact where exactness is
   available, enclosed where it is not, refused where honesty demands it. That
   is a picture of five claims, not a sentence. */
{
  const TOK = { exact: 'var(--c-2)', enclosed: 'var(--c-1)', refused: 'var(--c-3)' };
  const rows = [
    { k: 'S1 · total flows', note: 'CERTIFIED',
      segs: [{ x0: 0, x1: 1, token: TOK.enclosed, k: 'S1 total flows', v: 'CERTIFIED — the equilibrium totals are pinned' }] },
    { k: 'S1 · the split behind them', note: 'REFUSED',
      segs: [{ x0: 0, x1: 1, token: TOK.refused, hatch: true, k: 'S1 split',
               v: 'REFUSED — the decomposition is NOT unique, so no certificate is issued for it' }] },
    { k: 'S2 · equilibrium', note: 'EXACT',
      segs: [{ x0: 0, x1: 1, token: TOK.exact, k: 'S2', v: 'CERTIFIED EXACTLY — an exact rational solve, residual identically 0' }] },
    { k: 'S3 · equilibrium', note: 'r = ' + box,
      segs: [{ x0: 0, x1: 1, token: TOK.enclosed, k: 'S3', v: 'CERTIFIED — existence and local uniqueness in a box of radius ' + box }] }
  ];
  const fig = CH.segments({
    w: 900, rowH: 40, x0: 0, x1: 1, padL: 254, padR: 132,
    xTicks: [], rows,
    keys: [{ token: TOK.exact, t: 'exact — a rational identity, no width at all' },
           { token: TOK.enclosed, t: 'enclosed — an explicit certified radius' },
           { token: TOK.refused, t: 'refused — the object is not unique, so nothing is claimed', kind: 'hatch' }],
    alt: 'Four claims drawn from one published paper. The S1 totals and the S3 equilibrium are enclosed with '
      + 'certified radii, S2 is certified exactly with no width at all, and the S1 split is refused because the '
      + 'decomposition behind those totals is not unique.'
  });
  O.push(C.section({
    lab: '§0 · the grammar', title: 'One paper, and all three things a certificate can say',
    wide: true,
    bodyRaw: '<div class="col">'
      + C.pRaw('These four rows come out of a single published paper, and they are deliberately not all the '
        + 'same verdict. A reproduction that returned "confirmed" four times would be hiding the interesting '
        + 'row — the one where the object being reproduced is not unique, and the honest answer is to refuse.')
      + '</div>'
      + C.figure({ svgRaw: fig, caption: 'S2 is certified EXACTLY: the solve is rational and the residual is '
        + 'identically zero, so there is no radius to report. S1\'s totals and S3\'s equilibrium are enclosed '
        + '— S3 inside a Krawczyk box of radius ' + box + ' carrying existence AND local uniqueness. And S1\'s '
        + 'SPLIT is refused: the totals are unique but the decomposition into populations behind them is not, so '
        + 'there is nothing to certify and this page says so rather than reporting the first split its solver '
        + 'happened to land on.' })
  }));
}
...
O.push(C.section({
  ...
  bodyRaw: '<div class="col">'
    + C.pRaw('The <a href="https://github.com/carlostoledo1891/cert-machine/blob/main/legacy/research/wardrop-repro/wardrop-repro.html">published page</a> (byte-preserved in the repository, exactly '
      ...
      + '<a href="verify_wardrop.py">download the extracted verifier</a> and run <span class="m">python3 verify_wardrop.py</span> — '
      + 'under a second, exact Fractions end to end.')
    + '</div>'
}));

const foot = '<p>' + C.esc('Generated by tools/build-report-wardrop.js @ git ' + gitrev
  + ' — the embedded verifier extracted from the published bytes and re-run during this build (VERIFIED, or no page). '
  + 'The page as sent is byte-preserved in the repository (legacy/research/wardrop-repro/).') + '</p>'
  + '<p>' + C.esc('cert-machine · Carlos Toledo') + '</p>';

fs.writeFileSync(path.join(ROOT, 'reports', 'wardrop-repro.html'),
  TPL.render({ title: 'Wardrop, certified: exact, enclosed, refused · cert-machine', bodyRaw: O.join('\n\n') + CH.script(), footRaw: foot, path: '/reports/wardrop-repro.html',
    desc: 'A Wardrop traffic equilibrium enclosed, not just converged to: what a certificate adds to existence-by-algorithm, shown on a published paper’s own numbers.' }));
