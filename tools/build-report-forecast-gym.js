...
const { board, PROPOSERS } = require(path.join(ROOT, 'tools', 'forecast-gym.js'));
const B0 = board();
const meanW = (list) => {
...
const foot = '<footer class="col"><p>Generated by tools/build-report-forecast-gym.js @ git ' + git + '. Gates '
  ...
  + 'recounted string-exact, commit-before/score-after re-checked), and the admission board recomputed from '
  + 'the exact binomial tail. A deviation anywhere refuses the page. Forecasts on this page are proposals, '
  + 'never verdicts.</p></footer>';

fs.writeFileSync(path.join(ROOT, 'reports', 'forecast-gym.html'),
  TPL.render({ title: 'The Forecast Gym · cert-machine', bodyRaw: B.join('\n\n'), footRaw: foot, path: '/reports/forecast-gym.html',
