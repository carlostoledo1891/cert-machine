...
/* ---- the sheet, decided --------------------------------------------------
   Fifty-one printed rows and one exception. A table can say that; only a
   picture makes the ratio obvious at a glance, and the exception is labelled
   in words, never left to colour. */
{
  ...
  const fig = CH.strip({
    w: 900, perRow: 26, cell: 22, padL: 6,
    items: cells,
    keys: [{ token: 'var(--c-2)', t: hits.length + ' survive an unconditional audit' },
           { token: 'var(--c-1)', t: '1 REFUTED — ' + refuted.id }],
    alt: 'Fifty-one cells, one per printed row of the Ramanujan Machine sheets. Fifty are green — they survive '
      + 'an unconditional interval audit — and exactly one is plum: the refuted row, a sign slip in the printed '
      + 'mixed-zeta entry.'
  });
  B.push(C.section({
    lab: '§0 · the sheet', title: 'Every printed row, decided',
    wide: true,
    bodyRaw: '<div class="col">'
      + C.pRaw('One cell per PRINTED row. Our own corrected row is not counted here and never is — correcting a '
        + 'row is not the same as it having survived.')
      + '</div>'
      + C.figure({ svgRaw: fig, caption: printed.length + ' printed rows: ' + hits.length + ' survive an audit '
        + 'that assumes nothing (interval enclosures, no truncated-decimal collision anywhere), and exactly one '
        + 'is refuted — ' + refuted.id + ', a sign slip. The widest surviving enclosure is '
        + maxWidth.toExponential(2) + ', so the survivals are not near misses either. Hover any cell for its id, '
        + 'verdict and width.' })
  }));
}
...
const foot = '<p>Generated by tools/build-report-rm-audit.js @ git ' + git + '. All 52 '
  ...
  + 'and the build refuses on any deviation. Certificates and instruments: the cert-machine repository.</p>';

fs.writeFileSync(path.join(ROOT, 'reports', 'rm-audit.html'),
  TPL.render({ title: 'The Ramanujan Machine, audited', bodyRaw: B.join('\n\n') + CH.script(), footRaw: foot, path: '/reports/rm-audit.html',
