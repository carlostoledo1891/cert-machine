...
const pre = (txt) => '<div class="wide"><pre style="font-family:' + mono + ';font-size:13px;line-height:1.7;'
  + 'padding:14px 18px;border:1px solid var(--rule);border-radius:6px;overflow-x:auto;margin:0">' + C.esc(txt) + '</pre></div>';
...
B.push(C.section({
  ...
  bodyRaw: machineFlow(ledger, { gates: null, self: 'landing' })
    + '<div class="col">' + C.pRaw('The <a href="machine/">control page</a> is this drawing, live: every family, '
      + 'every battery executed at build time (never remembered), the full ledger decomposition, drift status.') + '</div>'
}));
