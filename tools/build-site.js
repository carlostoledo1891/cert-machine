...
const foot = '<footer class="col"><p>Generated by tools/build-site.js @ git ' + git + ' from ledger.json and the certificates — '
  ...
  /* the one link out to the ungated half. It is one link and not a section
     because /playground certifies nothing, and a page that leads with its
     refusals should not put an uncertified thing beside them. */
  + '<p style="margin-top:var(--sp-3);color:var(--ink-4)">There is also a <a href="/playground/">playground</a> — the same instruments with none of the gates, '
  + 'where nothing is certified and the pages say so.</p></footer>';
...
/* site/playground/ is the same arrangement for the opposite reason. It is
   emitted by playground/build.js, which is not wired into this file and to
   which none of this file's gates apply, because nothing under /playground
   ...
   files, certificates and verifiers are crawlable but are not pages. */
{
  ...
  for (const e of (() => { try { return fs.readdirSync(path.join(SITE, 'playground'), { recursive: true }); } catch (e) { return []; } })()) {
    const rel = String(e).split(path.sep).join('/');
    if (rel.endsWith('index.html')) urls.push('/playground/' + rel.slice(0, -'index.html'.length));
  }
...
for (const e of fs.readdirSync(SITE, { recursive: true })) {
  ...
  if (rel.startsWith('apps/') || rel.startsWith('playground/')) continue;
