...
const CERTS = [
  ...
  ['lambda4-phase0.json', 'The lambda(4) campaign, Phase 0: Mercer\'s Section-5 strategy (INTEGERS 19 (2019) #A4) executed mechanically — lambda(2) and the whole lambda(3) proof re-derived with exception families discovered and thresholds derived, not transcribed; the lambda(4) generic case with its 14 exception conditions discovered and matched against the paper\'s hand-written list; and the measured reduction: five of the fourteen carry strictly negative delta, so NINE families remain. Re-derived symbolically at every build by the lambda4 battery; the finite parts pinned here and sampled at every run.', null],
