/* build.js — SkyAudit's gated build: battery green, every city's day
   re-certified and compared against its committed records (refuses on
   drift), replay bundles regenerated, then the app is emitted —
   NYC (flagship) to site/apps/skyaudit/, the SP city pack (the
   Embraer/Eve jurisdiction demo, decided under ANAC RBAC 91.151(b))
   to site/apps/skyaudit/sp/.  apps/skyaudit · cert-machine             */
'use strict';
...
const { CITY_RULES, primaryRule } = require('./audit/corpus.js');
...
/* gates 2–8, PER CITY — each record regenerated and compared; a
   deviation restores the committed record and refuses, so drift
   refuses EVERY run */
const R = {};              /* R[city] = { after, bundle, refly, optimize, economics, stories, planner } */
function gateRecord(city, label, file, regen) {
  const p = path.join(dataDir, file);
  const before = fs.existsSync(p) ? JSON.parse(fs.readFileSync(p, 'utf8')) : null;
  regen();
  const fresh = JSON.parse(fs.readFileSync(p, 'utf8'));
  if (before && JSON.stringify(before) !== JSON.stringify(fresh)) {
    fs.writeFileSync(p, JSON.stringify(before, null, file.endsWith('summary.json') ? 2 : 1));
    die(city + ' ' + label + ' deviates from the committed record');
  }
  return fresh;
}
...
for (const city of ['nyc', 'sp']) {
  ...
  r.after = gateRecord(city, 'audit summary', city + '.audit-summary.json',
    () => run('node', ['audit/certify-day.js', city]));

  console.log('gate: replay bundle ' + city);
  run('node', ['audit/replay-bundle.js', city]);
  r.bundle = JSON.parse(fs.readFileSync(path.join(dataDir, city + '.replay.json'), 'utf8'));
  if (r.bundle.flights.length !== r.after.flights) {
    die(city + ' bundle carries ' + r.bundle.flights.length + ' flights, audit certified ' + r.after.flights);
  }

  console.log('gate: refly frontier ' + city);
  r.refly = gateRecord(city, 'refly frontier', city + '.refly.json',
    () => run('node', ['sim/refly.js', city]));

  console.log('gate: optimize thresholds ' + city);
  r.optimize = gateRecord(city, 'optimize thresholds', city + '.optimize.json',
    () => run('node', ['sim/optimize.js', city]));

  console.log('gate: economics ' + city);
  r.economics = gateRecord(city, 'economics', city + '.economics.json',
    () => run('node', ['audit/economics.js', city]));

  console.log('gate: stories ' + city);
  r.stories = gateRecord(city, 'day stories', city + '.stories.json',
    () => run('node', ['audit/stories.js', city]));

  console.log('gate: planner ' + city);
  r.planner = gateRecord(city, 'flight planner', city + '.planner.json',
    () => run('node', ['sim/planner.js', city]));
}

/* gate 9 — every city's tiles are the pinned bytes */
console.log('gate: tiles');
const tilesPins = JSON.parse(fs.readFileSync(path.join(APP, 'data/tiles/TILES-PINS.json'), 'utf8'));
for (const [city, pin] of Object.entries(tilesPins.cities)) {
  const p = path.join(APP, 'data/tiles', pin.file);
  const sha = require('crypto').createHash('sha256').update(fs.readFileSync(p)).digest('hex');
  if (sha !== pin.sha256) die(city + ' basemap tiles drifted from TILES-PINS.json');
  if (!pin.served_from) die(city + ' tiles pin has no served_from URL');
}
...
const RULE_SHORT = { 'faa-sfar-vfr': 'FAA 20-min', 'anac-rbac91-vfr': 'ANAC 20-min', 'easa-final-reserve': 'EASA 5-min' };
...
const CITY_TEXT = {
  nyc: {
    ...
    flown (${(a.flightStats.totalPathKm / 40075).toFixed(1)}× around the Earth)</b> — ADS-B,
    ...
    these same heliports in April 2026. <b>Click any aircraft — its certificate opens on the left.</b>`,
    rulesFine: 'FAA = the 20-min VFR helicopter tier; EASA = the 5-min final reserve, a NECESSARY condition only.',
  },
  sp: {
    title: 'SkyAudit São Paulo — the largest urban helicopter market, decided under ANAC rules',
    description: 'Real ADS-B helicopter traffic over São Paulo (2026-08-26), replayed — every flight audited against published eVTOL specs under ANAC RBAC 91.151(b), with mathematically certified enclosures.',
    ...
    ${a.flightStats.totalPathKm.toLocaleString('en-US')} km flown</b> — ADS-B, hash-pinned. Each
    flight is re-flown on paper by an eVTOL and decided by interval arithmetic <b>under Brazil's own
    reserve rule, ANAC RBAC 91.151(b)</b>. This is Embraer's home market: Eve's launch operator Revo
    holds a binding SP order (10 firm + 40 options, June 2025), and ANAC's vertiport sandbox runs at
    Campo de Marte — the busiest measured node on this map after Helicidade. Eve publishes the least
    of the four audited aircraft, so its column reads NEEDS DATA — and the flip threshold is computed,
    not rhetorical: <b>at ${r.optimize.battery_floor['eve-100'].targets['0.5'].kwh} kWh of published
    nameplate, half this day's flights become provable for the EVE-100</b> (proved on both sides of
    the flip, like every threshold here). The public UNDECIDABLE is the door.
    <b>Click any aircraft — its certificate opens on the left.</b>`,
    rulesFine: 'ANAC = RBAC 91.151(b), the 20-min VFR helicopter reserve, quoted from ANAC\'s own pinned text (arithmetically identical to the FAA tier — the jurisdiction is the point); EASA = the 5-min final reserve, a NECESSARY condition only.',
  },
};

/* ---- emit both cities ---- */
const { renderApp } = require(path.join(ROOT, 'design/app-shell.js'));
const rootOut = path.join(ROOT, 'site/apps/skyaudit');

for (const city of ['nyc', 'sp']) {
  const T = CITY_TEXT[city];
  const { after, refly, optimize, economics, stories, planner } = R[city];
  const PR = primaryRule(city);
  const outDir = city === 'nyc' ? rootOut : path.join(rootOut, 'sp');
  const prefix = city === 'nyc' ? '' : '../';        /* sp reuses the flagship's vendor/ + app.js */
  const urlBase = city === 'nyc' ? '/apps/skyaudit/' : '/apps/skyaudit/sp/';
  ...
  if (city === 'nyc') {
    ...
    fs.rmSync(path.join(outDir, 'tiles'), { recursive: true, force: true });   /* tiles serve from the repo raw URL */
  }

  fs.copyFileSync(path.join(dataDir, city + '.replay.json'), path.join(outDir, 'data', city + '.replay.json'));
  run('node', ['audit/ambient-bundle.js', city]);      /* decor; keeps previous bundle if full.jsonl absent */
  ...
  const pin = tilesPins.cities[city];
  const style = JSON.parse(fs.readFileSync(path.join(APP, 'src/style.json'), 'utf8'));
  style.sources.protomaps.url = 'pmtiles://' + pin.served_from;
  fs.writeFileSync(path.join(outDir, 'style.json'), JSON.stringify(style));

  const counts = after.bySpecRule['beta-alia|' + PR];
  const frontierRows = ['beta-alia', 'joby-s4', 'archer-midnight', 'eve-100'].map((s) => {
    const k = refly.keys[s + '|' + PR] || { certifiedLegs: 0, fleetMin: 0 };
    return k.certifiedLegs
      ? `<div class="as-frow"><span><span class="name">${NAMES[s]}</span>
         <span class="sub">${k.certifiedLegs} provable legs · witness ${k.witnessLocal} · pool model, ${RULE_SHORT[PR]}</span></span>
         <span class="val">exactly ${k.fleetMin} aircraft</span></div>`
      : `<div class="as-frow"><span><span class="name">${NAMES[s]}</span>
         <span class="sub">zero provable legs under ${RULE_SHORT[PR]}</span></span>
  ...
  const html = renderApp({
    ...
    homeHref: '/',
    navLinks: [
      { href: T.switcher.href, label: T.switcher.label },
      { href: '/reports/', label: 'reports' },
      { href: 'https://github.com/carlostoledo1891/cert-machine', label: 'github' }],
    mapAria: T.mapAria,
    styles: [prefix + 'vendor/maplibre-gl.css'],
    ...
    scripts: [prefix + 'vendor/maplibre-gl.js', prefix + 'vendor/deck.min.js', prefix + 'vendor/pmtiles.js', prefix + 'app.js'],
    leftHtml: `
    ...
    <section class="as-card">
      ...
      <div class="as-note">${T.auditNote(after, R[city])}</div>
      ...
      (quality-flagged in the repo; Eve publishes least, so Eve needs data most). ${T.rulesFine}
      Ambient layer = all other traffic, decor only. Data © <a href="https://adsb.lol">adsb.lol</a>
      contributors (ODbL). Rerun everything:
    ...
    <section class="as-card" style="border-style:dashed;border-color:var(--ink-3)">
      ...
      ${fOpen.map((c2) => {
        ...
        return `<div class="as-frow"><span><span class="name as-mono">${c2.id.split(':')[1]} · ${c2.target}</span>
  ...
  console.log('skyaudit ' + city + ' built green: ' + R[city].after.flights + ' flights, ' +
    counts.CERTIFIED + ' certified (beta-alia|' + PR + ')');
}
console.log('skyaudit built green: both city packs emitted');
