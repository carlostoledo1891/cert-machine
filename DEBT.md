...
|---|---|---|
| **the layout ruler and the render gate FLAKE inside `make test` when it runs right after `make site`** — twice on 2026-09-06 (`measure (layout ruler)` FAIL, once with `render` too), each time passing alone immediately afterwards and passing 76/76 on an unchanged re-run. **2026-09-07: it HUNG** — `check-measure.js --accept` sat 14 minutes with its headless Chrome alive and no output, was killed, and completed in 78 s on the next run. The headless binary had changed under the session: the Chrome helpers were version 151.0.7922 on 2026-09-06 and 152.0.7977 on 2026-09-07 (an auto-update), which is the first suspect now | not diagnosed beyond that; the CDP client has no watchdog, so a Chrome that never settles is a gate that never returns | an hour with the CDP client: a per-page timeout that REFUSES with the page named, the Chrome version printed in every run, and the two Chrome gates serialised behind one launcher · **2026-09-07, eighth session: three more hangs** (check-measure --accept once, check-render twice, each a Chrome with ~1 s CPU that never returned; killed and rerun clean each time; two chained runs were lost to it) |
| **the layout ruler's ratchet accepts a FLAKY LOW measurement as an improvement, then refuses the honest one as a regression** — found 2026-09-07 (ninth session): the `--accept` at 33e4f44 recorded `site/reports/zeta3-audit.html` as 3 spines @1440 / 3 @768 / 1 @390 with nothing clipped, where twelve consecutive baselines since 2026-09-05 read 7 / 7 / 5 with the table clipped at 768 and 390, and the page's bytes had not changed since 2026-09-05. The next three runs measured the true 7 / 7 / 5 (the probe run by hand, early and after a 3 s settle, fonts loaded, names every edge: page column, table border, the scope blockquote, `div.eq`, `div.note`, `ul.plain`, the nav links) and the ratchet called it a regression. Same Chrome flake as the row above, in its other costume: a page measured half-laid-out reads BETTER, and the ratchet only turns down | the row was restored by hand to the twelve-build value and the ruler passes 984 numbers; the defect is that `--accept` cannot tell an improvement from a degraded read | an hour in `check-measure.js`: `--accept` refuses to lower a page's row whose built bytes did not change since the baseline (a sha per row), or by more than one spine per run without `--accept-better` and a reason; and the same per-page timeout/serialisation as the row above |
