/* EMBER ivspecial.js — interval special functions for the MPS certificate.
   Built on lib/eqcert (interval + transcendental). MIT, ours.

   Exports (all take/return eqcert [lo,hi] intervals unless noted):
     sqrtIv       — interval sqrt (Math.sqrt is correctly rounded per ES spec;
                    1-ulp outward widening)
     atanIv       — arctan on [0,∞) via halving reduction + alternating series
     angleOf      — interval angle in [0, 2π) of a float vector (dx, dy)
     powIv        — x^p for x>0, real interval p: exp(p·log x)
     gammaIv      — Γ(z) for z ≥ 1 via Spouge (a = 20) with a defensively
                    widened error term; verified against exact values in
                    test-ivspecial.js (integers, half-integers via √π)
     besselJIv    — J_ν(x), ν ≥ 0 interval, x ≥ 0 interval, by power series
                    with certified alternating-tail bound
     besselJdIv   — d/dx J_ν(x), same series differentiated (same single Γ)

   Rigor notes:
   - The Bessel series terms use ONE Γ(ν+1); successive terms via the exact
     ratio −(x/2)²/(m(ν+m)), so Γ never re-enters.
   - Tail: once |t_{m+1}| ≤ q·|t_m| with certified q < 1, the alternating
     remainder is bounded by |t_{M+1}|/(1−q).
   - Spouge coefficients c_k = (−1)^{k−1}(a−k)^{k−1}√(a−k)·e^{a−k}/(k−1)!
     are built from exact integers × interval sqrt/exp. The bracket-sum
     cancellation (Σ|c_k| ~ 1e9) limits Γ widths to ~1e-7 relative — ample
     for defect certification (which needs ~1e-3 relative at f ~ 1e-5). */
'use strict';
...
/* ---------- Bessel J_ν(x), ν ≥ 0, x ≥ 0, x below ~30 ---------- */
function besselJIv(nu, x, terms) {
  const M = terms || 40;
  if (nu[0] < -0.999) throw new Error('besselJIv: nu > -1 required');
...
function besselJdIv(nu, x, terms) {
  ...
  // m=0 term: ν (x/2)^{ν−1} / (2 Γ(ν+1)); careful ν=0 → term is 0
  const x2 = mul(xh, xh);
  let s = I.ZERO;
  // base for term m: b_m = (x/2)^{2m+ν−1}/(m! Γ(m+ν+1)); build recursively
  let bm; // b_0 = (x/2)^{ν−1}/Γ(ν+1)
  if (nu[0] >= 1 || nu[1] <= 1) {
    bm = div(powIv(xh, sub(nu, I.ONE)), g);
  } else {
    bm = div(powIv(xh, sub(nu, I.ONE)), g); // powIv handles any real exponent, x>0
  }
  let t = null;
  for (let m = 0; m <= M; m++) {
  ...
  const q = I.mag(div(x2, mul(iv(M + 2), add(nu, iv(M + 2)))));
  if (!(q < 1)) throw new Error('besselJdIv: tail ratio not < 1');
