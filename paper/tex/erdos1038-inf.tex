...
\section{An independent audit of the family's covering}\label{sec:family-covering}

The $\varepsilon$-family claim is quantified over an interval and proved chunk
by chunk, so it stands on whether the chunks cover that interval. Recording only
a chunk count leaves the covering resting on the shape of the producing loop,
where no reader can check it.

The certificate therefore records the \emph{rung ladder}: the outer
subdivision, in which consecutive rungs share endpoints by construction and a
rung is marked covered only when it certifies as a single chunk or both of its
recursive halves do. That is a complete covering proof in 114
entries rather than 624,275,
and it is audited by a checker sharing no code with the producer: the rungs tile
$[10^{-12}, 10^{-1}]$ with no gap, every rung is covered, the per-rung chunk
counts sum to the reported total, and the number of failed chunks is zero and
stated explicitly rather than merely absent.
