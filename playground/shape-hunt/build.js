...
<section class="sec"><div class="wrap">
  ...
  <p class="why mt6"><b>All three models, the same element, on the same set.</b> Ground floor with the top floor, one with eight, two with seven. Nobody asked whether the building was symmetric — the models were asked, one pair at a time, how far apart two floors are, and the answers came back invariant under turning the building upside down.</p>
  <div class="words">
    ${canonRank.map(([c, k]) => {
      const ex = nmReal.filter((r) => r.best.canon === c).sort((a, b) => a.best.defect - b.best.defect)[0];
      return `<div class="w"><div class="wk"><b>${c}</b><br>${k} of ${nmReal.length} cases</div><div class="wv">${ex.best.words}<br><span class="ink-5">strongest on ${ex.set} · ${short(ex.model)} at a defect of ${n(ex.best.defect)}, where the generated label is <em>${ex.best.kind}</em></span></div></div>`;
    }).join('')}
  </div>
</div></section>
