...
.pend { opacity:.62; }
.wall { grid-template-columns: var(--c1) repeat(var(--cols), minmax(0, 1fr)); }
...
const deformWall = (subId) => `<div class="wall" style="--cols:${moodsRun.length};--c1:80px">
...
const verdictTable = () => `<div class="grid-t" style="--cols:3;--c1:260px">
...
const salienceTable = () => `<div class="grid-t" style="--cols:3;--c1:200px">
  <div class="h"></div>${MODELS.map(m => `<div class="h">${m.label}</div>`).join('')}
...
<section class="section">
  <div class="container">
    ...
    <p class="reveal mt4 rd ink-4 small">Cycle residuals then: 1%, 13%, 29%. Now: ${MODELS.map(m => pct(cell('neutral', 'clock', m).fits.cycle.resid)).join(', ')}. Nothing was carried over between the two runs but the question.</p>
  </div>
</section>
...
<section class="section">
  <div class="container narrow">
    <div class="prose reveal">
      ...
      <p class="mono ink-4 cmd">node experiments/neural-geometry/probe-mood.js --live<br>node experiments/neural-geometry/decide-mood.js<br>node tools/build-sentiment.js</p>
